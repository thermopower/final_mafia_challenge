...
1. `/docs/{requirement,prd,userflow,database,common-modules}.md` 문서를 읽고 프로젝트의 기획을 구체적으로 파악한다. `/doc/rules/tdd.md`를 읽고, TDD룰에 대해 파악한다.
2. `/docs/pages/{page-name}/{plan,state}.md`, `/docs/usecases/{00N}/spec.md` 문서를 읽고 구현 관련 세부사항을 파악한다.
3. 핵심 파악 및 요약 보고
