...
3. [기능별 상세 Userflow](#기능별-상세-userflow)
   ...
   - 3.3 [Excel 파일 업로드 플로우](#33-excel-파일-업로드-플로우)
...
- **핵심 목적**: 이카운트 시스템에서 추출한 데이터를 직관적으로 시각화하여 의사결정 지원
...
### 사용자 역할
- **관리자 (Admin)**: Excel 파일 업로드, 모든 데이터 조회 및 관리 가능
- **일반 사용자 (User)**: 대시보드 조회, 데이터 조회만 가능
...
│  ┌───────────────────────────────────────────────────┐  │
│  │ 날짜      파일명        유형    상태    작업      │  │
│  │ 2024-11-01  실적.xlsx   실적    성공    [삭제]   │  │
│  │ 2024-10-28  논문.xlsx   논문    성공    [삭제]   │  │
│  │ 2024-10-25  예산.xlsx   예산    실패    [재시도] │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```
